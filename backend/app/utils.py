...
def recipe_to_out(recipe: Recipe) -> dict:
    ...
    if getattr(recipe, "structured_ingredients", None):
        try:
            structured_ingredients = json.loads(recipe.structured_ingredients)
        except Exception:
            structured_ingredients = []
    ner_entities = []
    if getattr(recipe, "ner_entities", None):
        try:
            ner_entities = json.loads(recipe.ner_entities)
        except Exception:
            ner_entities = []
    return {
        "id": recipe.id,
